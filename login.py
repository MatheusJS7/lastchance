...
st.markdown("""
    <style>
        .stApp {
            background: linear-gradient(to bottom right, #FF7F50, #FF6347); /* Gradiente de fundo */
            color: #fff;
...
# Carrega credenciais diretamente do arquivo JSON
...
if "usuario_logado" not in st.session_state:
    ...
    # Campos de login com keys exclusivas
    user = st.text_input("Usuário", key="usuario_key")
    pwd = st.text_input("Senha", type="password", key="senha_key")
    
    # Adicionando animação de carregamento enquanto faz login
    if st.button("Entrar", key="entrar_button"):
        with st.spinner("Verificando usuário..."):
            if verificar_login(user, pwd):
                st.session_state.usuario_logado = user
                st.success(f"Bem-vindo, {user.capitalize()}! 🚀")
                st.balloons()  # Efeito de balões ao logar
                st.session_state["login_realizado"] = True  # Atualizar o estado de login
            else:
                st.error("Usuário ou senha incorretos. Tente novamente.")
                st.warning("Lembre-se: o usuário e a senha são sensíveis a maiúsculas/minúsculas.")
        st.stop()
...
if "usuario_logado" in st.session_state:
    ...
    with col2:
        if st.button("🚪 Sair"):
            ...
            st.session_state["login_realizado"] = False  # Resetar o login
            st.rerun
